...
const AddUser = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        try {
            ...
            payload.append("name", name.toString() || "")
            payload.append("email", email.toString() || "")
        ...
        <div>
            ...
            <Modal isShow={isShow} onClose={state => setIsShow(state)}>
               <form onSubmit={handleSubmit}>
                   ...
                   {/* modal body */}
                   <div className="p-5">
                    <InputGroupComponent className="text-black" id={`name`} type="text" value={user.name.toString()}
                           onChange={val => setUser({ ...user, name: val })}
                           required={true} label="Name" />

                    <InputGroupComponent className="text-black" id={`email`} type="email" value={user.email.toString()}
                           onChange={val => setUser({ ...user, email: val })}
                           required={true} label="Email" />

                    <InputGroupComponent className="text-black" id={`password`} type="password" value={user.password.toString()}

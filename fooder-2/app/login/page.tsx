...
const handleSubmit = async (e: FormEvent) => {
    try {
        ...
        if (data.status == true) {
            ...
            else if (role === `ADMIN`) setTimeout(() => router.replace(`/admin/dashboard`), 1000)
        }
        ...
        <div className="w-screen h-screen bg-login bg-cover">
            ...
            <div className="w-full h-full bg-backdrop-login flex justify-center items-center p-5">
                <div className="backdrop-blur-lg w-full md:w-6/12 lg:w-4/12 min-h-[600px] border rounded-xl bg- p-5 flex flex-col items-center relative">
                    ...
                    <Image alt="moklet-app" width={150} height={100} src={`/image/download (1).jpeg`}
                       ...
                       <div className="my-10">
                           <button type="submit"  className="bg-gray-500 hover:bg-primary uppercase w-full p-2 rounded-md text-white">
                               <a href="/manager/dashboard">Login</a>
                           </button>
                       </div>
                   </form>

...
import Logo from '../../public/image/download (1).jpeg'
import Profile from '../../public/image/download (2).jpeg'
...
const Sidebar = ({ children, id, title, menuList }: ManagerProp) => {
    ...
    return (
        <div className="w-full min-h-dvh bg-gray-400">
            {/* header section */}
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-black rounded-md shadow-lg py-1 z-10 top-full">
              <a href="/manager/profile" className="block px-4 py-2 text-sm text-white hover:bg-gray-100 hover:text-black">Profile</a>
              <a href="/manager/setting" className="block px-4 py-2 text-sm text-white hover:bg-gray-100 hover:text-black">Settings</a>
              <a onClick={handleLogout} href="/login" className="block px-4 py-2 text-sm text-white hover:bg-gray-100 hover:text-black">Logout</a>
            </div>
          )}
        </div>
            </header>
           ...
           bg-slate-600 border-r border-primary ${isShow ? `translate-x-full` : ``}`}>
                ...
                <div className="mb-3 w-full flex justify-center">
                    <div className="flex items-center space-x-2">
                        ...
                        <h1 className="text-2xl font-bold text-white-500">Amba Web</h1>
                    </div>
                </div>
                {/* end logo section */}
